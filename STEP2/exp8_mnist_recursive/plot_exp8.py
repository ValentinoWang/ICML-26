...
def plot_series(
    ...
    for m, label in [
        ...
        line, = axes[0].plot(gens, mean, label=label, linewidth=2)
    ...
    for m, label in [
        ...
        ci = 1.96 * std / np.sqrt(n_seeds)
        axes[1].plot(gens, mean, label=label, linewidth=2)
...
def plot_series_main(
    ...
    gens = np.arange(1, len(next(iter(stats["mse"].values()))["mean"]) + 1)
    methods = [
        ("mlp_filter", "Pointwise (MLP)", "#d62728", "-.", 2),
        ("batch_stats", "Pointwise + Batch Stats (\u2248 MLP)", "#ff7f0e", ":", 2),
        ("ours", "Set-Aware", "#1f77b4", "-", 3),
    ]
    ...
    axins.plot(gens, mean, linewidth=1.2, color="#1f77b4")
