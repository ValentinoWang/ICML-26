...
def apply_clean_val_geometry(
    weights: torch.Tensor,
    embeddings: torch.Tensor,
    clean_embeddings: torch.Tensor | None,
    scale: float,
) -> torch.Tensor:
    if clean_embeddings is None or scale == 0.0:
        return weights
    if embeddings.numel() == 0 or clean_embeddings.numel() == 0:
        return weights

    cand = embeddings.to(weights.device)
    clean = clean_embeddings.to(weights.device)
    cand_mean = cand.mean(dim=0, keepdim=True)
    clean_mean = clean.mean(dim=0, keepdim=True)
    drift = cand_mean - clean_mean
    drift_norm = drift.norm(dim=-1, keepdim=True)
    if torch.any(~torch.isfinite(drift_norm)) or float(drift_norm.max().item()) <= 1e-8:
        return weights

    cand_unit = F.normalize(cand, dim=-1)
    drift_unit = F.normalize(drift, dim=-1)
    align = (cand_unit * drift_unit).sum(dim=-1)
    weights_safe = weights.clamp(1e-6, 1.0 - 1e-6)
    logits = torch.logit(weights_safe)
    return torch.sigmoid(logits - float(scale) * align)
...
def select_training_indices(
    ...
    if method != "unsup_set_aware":
        ...
        w = apply_clean_val_geometry(
            w,
            embeddings=embeddings,
            clean_embeddings=clean_embeddings,
            scale=float(filter_args.get("clean_val_geom_scale", 0.0)),
        )
    del filter_model
