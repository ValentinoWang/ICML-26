...
结果来自：
- `STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_baseline_balanced_train_holdout_cleanval/`
- `STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_baseline_score_topk_train_holdout_cleanval/`（严格对照：同 v3g 的 \texttt{score\_topk} 规则，但 score=confidence）
- `STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_setaware_tuned_v3g_dphi0/`
- `STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_setaware_tuned_v3g/`
...
结果来自：
- `STEP2/exp11_gpt2_model/Results/dphi1/`（no_filter / pointwise / set_aware）
- `STEP2/exp11_gpt2_model/Results/dphi0/`（set_aware，Δϕ=0）
- `STEP2/exp11_gpt2_model/Results/dphi1_leash/`（set_aware + PPL leash）
- `STEP2/exp11_gpt2_model/Results/dispersion/`（新增：非学习几何 baseline，dispersion 直接算权重）
- `STEP2/exp11_gpt2_model/Results/ppl_safety/`（新增：点式“只避险” baseline，仅剔除极高 PPL）
- `STEP2/exp11_gpt2_model/Results/rep_filter_thr0.6/`（新增：硬规则 n-gram 重复过滤 baseline）
- `STEP2/exp11_gpt2_model/Results/pointwise_mix0.5/`、`STEP2/exp11_gpt2_model/Results/pointwise_mix0.2/`（新增：pointwise + mix 原始数据）
- 质量侧指标汇总：`STEP2/exp11_gpt2_model/Results/analysis_text_quality/train_g4.csv`
...
（已修）论文 `STEP2/Paper/LaTEX/icml2025/main.tex` 已切换到当前可复现目录 `STEP2/exp11_gpt2_model/Results/dphi1/` 与 `STEP2/exp11_gpt2_model/Results/dphi1_leash/`，并补充训练文本质量侧指标（`unique_line_ratio/rep4/gzip`）的证据链。
...
- 论文写法（`STEP2/Paper/LaTEX/icml2025/main.tex:539`）给出的结论是：
  ...
  - ESS≈8k（候选池 10k）
- 从 `STEP2/exp9_cifar10_setaware/results/exp9_seed*_merged.csv` 逐 seed 计算（gen5，3 seeds）：
...
- `STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05/exp9_seed*_merged.csv`
...
- 结果目录：`STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_baseline_balanced/`
- gen5（3 seeds）均值：
...
- v2 结果目录：`STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_setaware_tuned_v2/`
...
- 公平 baseline：`baseline_balanced`  
  - acc **0.4798±0.0298** / worst **0.2424±0.0396**（`STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_baseline_balanced/`）
- set_aware v2（dphi1）：  
  - acc **0.4850±0.0235** / worst **0.2522±0.0489**（`STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_setaware_tuned_v2/`）
- set_aware v2（dphi0）：  
  - acc **0.4776±0.0128** / worst **0.2411±0.0413**（`STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_setaware_tuned_v2_dphi0/`）
...
- **无 clean-val（完全不使用干净标签）**：检验 v2 的优势是否依赖 clean-val  
  - baseline_balanced：`STEP2/exp9_cifar10_setaware/run_baseline_balanced_alpha05_no_cleanval.sh` → `STEP2/exp9_cifar10_setaware/results_balance_alpha05_baseline_balanced_no_cleanval/`  
  - set_aware v2：`STEP2/exp9_cifar10_setaware/run_setaware_tuned_alpha05_v2_no_cleanval.sh` → `STEP2/exp9_cifar10_setaware/results_balance_alpha05_setaware_tuned_v2_no_cleanval/`
- **clean-val 来自 train holdout（不碰 test）**：检验 meta 机制是否能在“无 test 泄漏”的设定下仍然成立  
  - baseline_balanced：`STEP2/exp9_cifar10_setaware/run_baseline_balanced_alpha05_train_holdout_cleanval.sh` → `STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_baseline_balanced_train_holdout_cleanval/`  
  - set_aware v2：`STEP2/exp9_cifar10_setaware/run_setaware_tuned_alpha05_v2_train_holdout_cleanval.sh` → `STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_setaware_tuned_v2_train_holdout_cleanval/`

跑完后用一条命令汇总 gen5（3 seeds 均值±方差）：

`python STEP2/exp9_cifar10_setaware/summarize_results.py <DIR1> <DIR2> --gen 5`
...
- **无 clean-val（full test；不使用任何干净标签）**  
  - baseline_balanced：acc **0.4796±0.0300** / worst **0.2440±0.0392**（`STEP2/exp9_cifar10_setaware/results_balance_alpha05_baseline_balanced_no_cleanval/`）  
  - set_aware v2（dphi1）：acc **0.4837±0.0196** / worst **0.2543±0.0540**（`STEP2/exp9_cifar10_setaware/results_balance_alpha05_setaware_tuned_v2_no_cleanval/`）  
  - 差值：**Δacc=+0.0041**、**Δworst=+0.0103**
- **train holdout clean-val（meta on；full test；不碰 test 做 meta）**  
  - baseline_balanced：acc **0.4827±0.0148** / worst **0.2483±0.0476**（`STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_baseline_balanced_train_holdout_cleanval/`）  
  - set_aware v2 + meta（dphi1）：acc **0.4877±0.0189** / worst **0.2603±0.0414**（`STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_setaware_tuned_v2_train_holdout_cleanval/`）  
  - 差值：**Δacc=+0.0050**、**Δworst=+0.0120**
- **test clean-val（meta on；评测剔除 100 张 test）**（原 v2 配置对照）  
  - baseline_balanced：acc **0.4798±0.0298** / worst **0.2424±0.0396**（`STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_baseline_balanced/`）  
  - set_aware v2 + meta（dphi1）：acc **0.4850±0.0235** / worst **0.2522±0.0489**（`STEP2/exp9_cifar10_setaware/results_meta_balance_alpha05_setaware_tuned_v2/`）  
...
为确认代码路径，我们曾先跑过 gen=1 的最小对照（目录：`STEP2/exp9_cifar10_setaware/results_ctrl_no_cleanval_g1_seed1088/` 与 `STEP2/exp9_cifar10_setaware/results_ctrl_train_holdout_cleanval_g1_seed1088/`）。gen=1 的波动与 gen=5 的最终趋势并不完全一致，提示 meta 信号在早期可能存在不稳定性（建议在论文中做 meta warmup/退火并报告）。
...
- `STEP2/exp11_gpt2_model/Results/dphi1/`（generation=4）：
...
- `STEP2/exp11_gpt2_model/Results/dphi1_leash/`：
...
- 结果目录：`STEP2/exp11_gpt2_model/Results/dphi0/`
...
重要备注（审稿风险）：当前仓库 `STEP2/exp11_gpt2_model/generations/` 下已有的 `.txt` 是历史 dump，**行数显示它对应的训练规模约为 512（而非当前 dphi1/dphi0 的 2000）**，因此这些数值只能作为“pointwise 容易塌缩”的定性证据，不应直接写入论文作为最终结果。

为解决“对齐最终配置”的问题，我们已把这些质量侧统计 **直接写入主实验输出**：`STEP2/exp11_gpt2_model/run_exp11_gpt2_model.py` 会在每一代的 `metrics_diversity_ppl.{json,csv}` 里附带
...
- 历史 dump 的结果（gen=4 train；3 seeds；见 `STEP2/exp11_gpt2_model/Results/analysis_text_quality/train_g4_lines.csv`）：  
...
`python STEP2/exp11_gpt2_model/summarize_saved_generations.py --generations-root STEP2/exp11_gpt2_model/generations --gen 4 --split train --out STEP2/exp11_gpt2_model/Results/analysis_text_quality/train_g4_lines.csv`

### 3.2 leash “看起来没生效”的代码层原因（不是 bug，但必须在论文里解释/量化）

- leash 的实现是在 set-aware 输出的权重 `w` 上做逐样本惩罚 `w ← w * leash`，随后直接 `topk(w)` 选训练集：见 `STEP2/exp11_gpt2_model/filter_module.py:37` 与 `STEP2/exp11_gpt2_model/filter_module.py:200`。
...
- 代码位置：`STEP2/exp11_gpt2_model/filter_module.py` 与 `STEP2/exp11_gpt2_model/run_exp11_gpt2_model.py`
