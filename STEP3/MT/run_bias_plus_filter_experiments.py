...
"""
MT/Toy 数据集上的 YOLO + L_bias + g_phi 偏置控制实验入口
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="一键运行 YOLO + L_bias + MLP FiLTER (g_phi) 偏置控制实验（MT/Toy）"
    )
    parser.add_argument(
        "--scenarios",
        nargs="*",
...
def main() -> None:
    ...
    print("🎯 YOLO + L_bias + MLP FiLTER (g_phi) 偏置控制实验（MT/Toy，一键模式）")
