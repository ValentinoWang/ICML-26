...
def run(cmd: list[str], cwd: Path, *, dry_run: bool, env: dict[str, str]) -> None:
    printable = " ".join(cmd)
...
def main():
    ...
    # Ensure experiment scripts can import `bias/`, `filter/`, `Tools/` from the Release root.
    env = os.environ.copy()
    extra_paths = f"{release_root}:{root}"
    old = env.get("PYTHONPATH", "")
    env["PYTHONPATH"] = f"{extra_paths}:{old}" if old else extra_paths
    ...
    experiments: list[tuple[str, list[str]]] = [
        ...
        ("exp13_Qwen_model", [sys.executable, str(root / "exp13_Qwen_model" / "run_exp12_oneshot.py")]),
    ]
    ...
    for name in selected:
        run(exp_map[name], cwd=root, dry_run=args.dry_run, env=env)
