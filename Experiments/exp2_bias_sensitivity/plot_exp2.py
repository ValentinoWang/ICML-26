...
def plot_theory_validation_triptych(
    ...
    with plt.rc_context(PAPER_STYLE):
        ...
        ax.set_title(r"(a) Fixed Bias ($\beta=0.5$)")
        ax.set_xlabel("Generation")
        ax.set_ylabel(r"$\|\theta_t - \theta^*\|_2$")
        ...
        ax.set_title(r"(b) Ridge Sensitivity ($\alpha$)")
        ...
        ax.set_title(r"(c) Additive Bias Sensitivity ($\beta$)")
...
def regenerate_from_results(
    ...
    if exp1_const.exists() and bias_summary.exists() and ridge_summary.exists():
        out_path = ROOT / "Paper" / "LaTEX" / "icml2025" / "Figures" / "exp3_theory_validation.png"
