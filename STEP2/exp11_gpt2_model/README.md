...
python exp11_gpt2_model/run_exp11_gpt2_model.py --generations 5 --candidate-pool 10000 --train-samples 2000 --save-checkpoints
```
...
## Outputs
- 默认基路径 `STEP2/Total_results/Tables/exp11_gpt2_model/Results/metrics_diversity_ppl.json`（仓库根相对路径），并为每个 seed 写入 `<base_dir>/<seed>/<base_name>.{json,csv}`。如传入 `--results-path <path/to/your.json>`，会在 `<path/to>/<seed>/<your.json|.csv>` 下生成。Checkpoints 在对应 seed 目录内（开启 `--save-checkpoints` 时）。
