...
from plot_exp6 import plot_set_size_trajs, plot_tail_bar, plot_trajectories  # noqa: E402
...
def run_single_scenario(
    ...
    device: torch.device,
    seed: int,
    override_samples_per_gen: Optional[int] = None,
) -> Dict[str, List[float]]:
    rng = set_seed(seed)
    samples_per_gen = args.samples_per_gen if override_samples_per_gen is None else override_samples_per_gen
    if scenario == "ridge":
        dim = args.dim_ridge
        ...
        samples_per_gen = samples_per_gen if override_samples_per_gen is not None else (args.samples_per_gen_complex or samples_per_gen)
    else:
    ...
    for _ in range(args.generations):
        ...
        # Ours full: set-aware weights + correction
        w_full, delta_full = sa_model_full(x)
        theta_new_full = delta_full  # force w=1 in estimate; w_full only for auxiliary supervision
        loss_full = (
...
def save_set_size_csv(
    set_size_series: Dict[int, Dict[str, Dict[str, List[float]]]],
    set_size_tails: Dict[int, Dict[str, Dict[str, float]]],
    out_dir: pathlib.Path,
    scenario: str,
) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)
    sizes = sorted(set_size_series.keys())
    gens = len(next(iter(set_size_series.values()))["ours_full"]["mean"])

    traj_path = out_dir / f"exp6_setsize_{scenario}_trajectories.csv"
    with traj_path.open("w") as f:
        header = ["generation"]
        for n in sizes:
            header.extend([f"N{n}_mean", f"N{n}_std"])
        f.write(",".join(header) + "\n")
        for i in range(gens):
            row = [str(i + 1)]
            for n in sizes:
                row.append(f"{set_size_series[n]['ours_full']['mean'][i]:.6f}")
                row.append(f"{set_size_series[n]['ours_full']['std'][i]:.6f}")
            f.write(",".join(row) + "\n")

    tail_path = out_dir / f"exp6_setsize_{scenario}_tail_summary.csv"
    with tail_path.open("w") as f:
        header = []
        for n in sizes:
            header.extend([f"N{n}_mean", f"N{n}_std"])
        f.write(",".join(header) + "\n")
        row = []
        for n in sizes:
            row.append(f"{set_size_tails[n]['ours_full']['mean']:.6f}")
            row.append(f"{set_size_tails[n]['ours_full']['std']:.6f}")
        f.write(",".join(row) + "\n")
...
def main():
    ...
    parser.add_argument("--run-set-size-ablation", action="store_true", help="Run ridge regression set-size sweep for set-aware robustness.")
    parser.add_argument("--set-sizes", type=int, nargs="+", default=[8, 32, 128, 512], help="Candidate counts per generation for set-size sweep.")
    parser.add_argument("--set-size-scenario", type=str, choices=["ridge", "bayes", "complex"], default="complex", help="Scenario to sweep set size on.")
    ...
    set_size_series: Dict[int, Dict[str, Dict[str, List[float]]]] = {}
    set_size_tails: Dict[int, Dict[str, Dict[str, float]]] = {}
    set_size_time: Dict[int, float] = {}
    if args.run_set_size_ablation:
        set_scenario = args.set_size_scenario
            ...
            for s in seeds:
                seed_series.append(
                    run_single_scenario(
                        ...
                        device=device,
                        seed=s,
                        override_samples_per_gen=n,
                    )
                )
            set_size_series[n] = aggregate_seed_series(seed_series)
            set_size_tails[n] = aggregate_tail(seed_series, tail=args.tail_window)
            set_size_time[n] = time.time() - t0
            print(f"Set size N={n} {set_scenario} sweep done in {set_size_time[n]:.1f}s on {device}")
    ...
    if args.run_set_size_ablation and len(set_size_series) > 0:
        set_scenario = args.set_size_scenario
        plot_set_size_trajs(set_size_series, fig_dir / f"exp6_setsize_{set_scenario}.png", n_seeds=len(seeds), scenario=set_scenario)
        save_set_size_csv(set_size_series, set_size_tails, out_dir, scenario=set_scenario)
    ...
    if args.run_set_size_ablation and len(set_size_time) > 0:
        runtime["set_size_time_sec"] = set_size_time
