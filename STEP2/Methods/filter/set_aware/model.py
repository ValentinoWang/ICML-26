...
class SetAwareBiasRobustFilter(nn.Module):
    ...
    def __init__(
        ...
        # Optional gating head (used when calling forward_with_gate); kept here to avoid diverging architectures.
        self.gate_head = MLP(hidden, hidden, 1, dropout=dropout)
    ...
    def forward_with_gate(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Extended forward returning a gating scalar g in [0,1] predicted from global pooled features.
        """
        h0 = self.encoder(x)
        h = self.transformer(h0)
        weights = torch.sigmoid(self.weight_head(h)).squeeze(-1)
    ...
    def forward_with_gate(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Same as forward, additionally returns a gating scalar in [0,1] predicted from global pooled features.
        """
        weights, delta_phi = self.forward(x)
        # Recompute pooled from encoded transformer output for consistency
        h0 = self.encoder(x)
        h = self.transformer(h0)
        h_global = h.mean(dim=1)
        gate = torch.sigmoid(self.gate_head(h_global)).squeeze(-1)  # [B]
